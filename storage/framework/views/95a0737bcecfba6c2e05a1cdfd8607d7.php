...
<html lang="<?php echo e(str_replace('_', '-', app()->getLocale())); ?>" class="h-full">
    <head>
        ...
        <title><?php echo e(isset($title) && $title ? $title . ' | Rooterin - Sistem Operasional' : 'Rooterin - Sistem Operasional'); ?></title>
    ...
    <body class="h-full bg-[#f8f9fa] font-sans antialiased overflow-x-hidden">
        <div 
            ...
            @slide-over-loading-start.window="slideOverOpen = true; slideOverTitle = '<?php echo e(app()->getLocale() == 'en' ? 'Loading Details...' : 'Memuat Detail...'); ?>'; slideOverContent = ''; slideOverLoading = true"
        >
            <!-- Sidebar -->
                ...
                <header class="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm relative w-full">
                        ...
                        <div class="hidden sm:flex items-center gap-3">
                            <div class="flex flex-col">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[11px] font-bold text-slate-700"><?php echo e(__('ui.system_live')); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 md:gap-6 shrink-0">
                        ...
                        <div class="flex items-center bg-slate-50 rounded-lg p-1 border border-slate-200">
                            <a href="<?php echo e(route('lang.switch', 'id')); ?>" @click="$dispatch('close-chat'); $dispatch('close-chatbot')" class="px-2.5 py-1 text-[10px] font-black rounded-md transition-all <?php echo e(App::getLocale() == 'id' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'); ?>">ID</a>
                            <a href="<?php echo e(route('lang.switch', 'en')); ?>" @click="$dispatch('close-chat'); $dispatch('close-chatbot')" class="px-2.5 py-1 text-[10px] font-black rounded-md transition-all <?php echo e(App::getLocale() == 'en' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'); ?>">EN</a>
                        </div>
...
[$__name, $__params] = $__split('navbar-notification', []);
...
$__key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-3534514690-0', $__key);
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open; if (open) { $dispatch('close-chat'); $dispatch('close-chatbot'); }" class="flex items-center gap-3.5 group focus:outline-none">
                                <div class="flex flex-col text-right hidden md:flex">
                                    <span class="text-[11px] font-black text-slate-900 leading-tight"><?php echo e(Auth::user()->name); ?></span>
                                    <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider"><?php echo e(Auth::user()->role); ?></span>
                                </div>
                                <img src="<?php echo e(Auth::user()->profile_photo_url); ?>" alt="<?php echo e(Auth::user()->name); ?>" class="w-10 h-10 rounded-xl object-cover shadow-lg shadow-slate-900/10 ring-2 ring-transparent group-hover:ring-indigo-500 transition-all duration-300">
                            </button>
                            ...
                            >
                                <div class="px-4 py-4 border-b border-slate-100 mb-2">
                                    <p class="text-xs font-black text-slate-900 uppercase tracking-tight"><?php echo e(Auth::user()->name); ?></p>
                                    <p class="text-[10px] text-slate-500 font-bold truncate mt-0.5"><?php echo e(Auth::user()->email); ?></p>
                                </div>
                                <div class="space-y-1">
                                    <a href="<?php echo e(route('profile.edit')); ?>" class="flex items-center gap-3 px-4 py-2.5 text-[12px] font-bold text-slate-600 hover:bg-slate-50 rounded-xl transition-colors group">
                                        <i data-lucide="user" class="w-4 h-4 group-hover:text-indigo-500"></i> <?php echo e(__('Profile')); ?>

                                    </a>
                                    <a href="<?php echo e(route('security.center')); ?>" class="flex items-center gap-3 px-4 py-2.5 text-[12px] font-bold text-slate-600 hover:bg-slate-50 rounded-xl transition-colors group">
                                        <i data-lucide="shield" class="w-4 h-4 group-hover:text-indigo-500"></i> <?php echo e(__('ui.security_center')); ?>
                                ...
                                <form method="POST" action="<?php echo e(route('logout')); ?>">
                                    ...
                                    <button type="submit" class="flex items-center gap-3 w-full text-left px-4 py-2.5 text-[12px] font-bold text-rose-500 hover:bg-rose-50 rounded-xl transition-colors group">
                                        <i data-lucide="log-out" class="w-4 h-4 group-hover:translate-x-1 transition-transform"></i> <?php echo e(__('ui.logout') ?? 'End Session'); ?>
...
<?php /**PATH C:\laragon\www\Rooterin-Invoice\resources\views/layouts/app.blade.php ENDPATH**/ ?>