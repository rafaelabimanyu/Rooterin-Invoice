<x-app-layout :title="__('ui.edit_business_unit')">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-slate-900 font-outfit">{{ __('ui.edit_business_unit') }}</h1>
        <p class="text-slate-500 font-medium text-sm">{{ app()->getLocale() == 'en' ? 'Update the details and configuration of this business unit' : 'Perbarui rincian dan konfigurasi dari unit bisnis ini' }}</p>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-6">
                <div class="bg-white p-8 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
                    ...
                    <div class="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                        ...
                        <div class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $businessUnit->is_active) ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="p-6 bg-gold-50 rounded-3xl border border-gold-100">
                    <p class="text-[11px] text-gold-700 font-black mb-4 font-jakarta uppercase tracking-wider">{{ app()->getLocale() == 'en' ? 'Verify information before saving updates.' : 'Verifikasi informasi sebelum menyimpan perubahan.' }}</p>
                    <button type="submit" class="w-full py-4 bg-gold-500 hover:bg-gold-600 text-slate-950 rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-lg shadow-gold-500/25">
                        {{ app()->getLocale() == 'en' ? 'Update Business Unit' : 'Simpan Perubahan' }}
                    </button>
                </div>
            </div>
        </div>
