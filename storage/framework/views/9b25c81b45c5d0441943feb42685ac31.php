    ...
    <div class="bg-gradient-to-r from-indigo-50/50 to-blue-50/30 rounded-3xl border border-indigo-100/80 p-6 md:p-8 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row items-start gap-4 sm:gap-5 relative z-10 flex-1 w-full">
            ...
            <div class="space-y-2 flex-grow w-full">
                ...
                <h4 class="text-base font-bold text-slate-900 leading-snug font-jakarta"><?php echo e($locale == 'en' ? 'Financial Strategy & Cash Flow' : 'Taktik Keuangan & Arus Kas'); ?></h4>
                <div x-data="{ expanded: false }" class="mt-2">
                    ...
                    >
                        <span x-text="expanded ? '<?php echo e($locale == 'en' ? 'Show Less' : 'Lihat Lebih Sedikit'); ?>' : '<?php echo e($locale == 'en' ? 'Show More' : 'Lihat Selengkapnya'); ?>'"></span>
        ...
        <div class="flex items-center gap-3 shrink-0 relative z-10 w-full md:w-auto">
            <button wire:click="refreshAnalysis" class="w-full md:w-auto justify-center px-4 py-2.5 bg-white border border-slate-200 hover:border-indigo-200 hover:text-indigo-600 rounded-xl text-xs font-bold shadow-sm transition-all flex items-center gap-2 active:scale-95">
                <span wire:loading.remove wire:target="refreshAnalysis" class="flex items-center gap-2">
                    <i data-lucide="refresh-cw" class="w-3.5 h-3.5"></i>
                    <span><?php echo e($locale == 'en' ? 'Refresh Analysis' : 'Perbarui Analisis'); ?></span>
                </span>
                <span wire:loading wire:target="refreshAnalysis" class="flex items-center gap-2" x-cloak>
                    ...
                    <span><?php echo e($locale == 'en' ? 'Analyzing...' : 'Menganalisis...'); ?></span>
                </span>
            </button>
        </div>
    </div>
...
<?php /**PATH C:\laragon\www\Rooterin-Invoice\resources\views/livewire/dashboard/financial-advisory.blade.php ENDPATH**/ ?>