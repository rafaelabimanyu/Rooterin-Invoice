<style>
    ...
    .chatbot-content-html strong { font-weight: 700; color: inherit; }
    .chatbot-content-html code { background-color: rgba(0, 0, 0, 0.06); padding: 0.1rem 0.25rem; border-radius: 0.25rem; font-family: monospace; font-size: 0.9em; }
...
<div x-data="{ open: false, maximized: false }" class="fixed bottom-6 right-6 z-50">
    ...
    <div 
        ...
        class="absolute bottom-20 right-0 bg-white/90 backdrop-blur-md border border-slate-200/70 shadow-[0_32px_64px_rgba(15,23,42,0.18)] overflow-hidden flex flex-col transition-all duration-300 ease-out-spring"
        :class="maximized ? 'w-[90vw] md:w-[580px] h-[680px] rounded-[2.25rem]' : 'w-[85vw] sm:w-[380px] md:w-[400px] h-[510px] rounded-[1.85rem]'"
        style="display: none;"
    >
        <!-- Header -->
        <div class="px-6 py-4.5 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800/85">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-500 to-violet-600 flex items-center justify-center text-white shadow-md border border-indigo-400/20">
            ...
            <div class="flex items-center gap-1.5">
                ...
                <button 
                    ...
                    class="p-1.5 hover:bg-white/10 rounded-lg text-slate-400 hover:text-white transition-all active:scale-95" 
        ...
        <div 
            x-ref="chatContainer" 
            class="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-50/50 chatbot-scrollbar"
        >
            @foreach($messages as $idx => $msg)
                <div class="flex items-start gap-2.5 w-full {{ $msg['sender'] === 'user' ? 'justify-end' : 'justify-start' }}">
                    <!-- AI Avatar (only for AI messages) -->
                    @if($msg['sender'] === 'ai')
                        <div class="w-7 h-7 rounded-lg bg-indigo-50 border border-indigo-150 flex items-center justify-center text-indigo-650 shrink-0 shadow-sm mt-0.5">
                    ...
                    <div class="flex flex-col {{ $msg['sender'] === 'user' ? 'items-end' : 'items-start' }} max-w-[80%]">
                        <!-- Message Bubble -->
                        <div 
                            class="px-4 py-3 rounded-2xl text-xs leading-relaxed {{ $msg['sender'] === 'user' ? 'bg-indigo-600 text-white rounded-tr-none shadow-md shadow-indigo-600/5 font-medium' : 'bg-white border border-slate-200/80 text-slate-800 rounded-tl-none shadow-sm' }}"
                        >
                        ...
                        @if(isset($msg['navigateUrl']) && $msg['navigateUrl'])
                            <div class="mt-2 pl-1">
                                <a 
                                    ...
                                    class="inline-flex items-center gap-1.5 px-3 py-2 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-600 rounded-xl text-[10px] font-black transition-all shadow-sm hover:-translate-y-0.5 active:scale-95"
                                >
                                    <i data-lucide="external-link" class="w-3.5 h-3.5"></i>
                    ...
                    @if($msg['sender'] === 'user')
                        <div class="w-7 h-7 rounded-lg bg-slate-900 flex items-center justify-center text-white shrink-0 shadow-sm mt-0.5">
            ...
            <div wire:loading wire:target="sendMessage" class="flex items-start gap-2.5">
                <div class="w-7 h-7 rounded-lg bg-indigo-50 border border-indigo-150 flex items-center justify-center text-indigo-600 shrink-0 shadow-sm mt-0.5 animate-pulse">
                    <i data-lucide="bot" class="w-4 h-4"></i>
                </div>
                <div class="bg-white border border-slate-200 text-slate-400 rounded-2xl rounded-tl-none px-4 py-3 shadow-sm flex items-center gap-1.5">
                    <span class="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce" style="animation-delay: 0.1s"></span>
                    <span class="w-1.5 h-1.5 bg-indigo-600 rounded-full animate-bounce" style="animation-delay: 0.2s"></span>
                    <span class="w-1.5 h-1.5 bg-indigo-700 rounded-full animate-bounce" style="animation-delay: 0.3s"></span>
                </div>
            </div>
        </div>
        ...
        <form 
            ...
            class="p-4 bg-white border-t border-slate-200 flex items-center gap-3 shrink-0"
            x-data="{
                handleSubmit() {
                    $nextTick(() => {
        ...
        >
            <input 
                ...
                class="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all text-slate-800 font-medium" 
            ...
            <button 
                type="submit" 
                class="w-10 h-10 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl flex items-center justify-center shrink-0 transition-all active:scale-95 disabled:opacity-50 shadow-md shadow-indigo-600/10 focus:outline-none"
                wire:loading.attr="disabled"
                wire:target="sendMessage"
            >
