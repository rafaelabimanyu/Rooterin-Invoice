...
<head>
    ...
    <title>rooterin.com — Ultimate Operational Excellence</title>
    ...
    <style>
        ...
        @keyframes maskSlide {
            from { clip-path: inset(0 100% 0 0); transform: translateX(-30px); opacity: 0; }
            to { clip-path: inset(0 0 0 0); transform: translateX(0); opacity: 1; }
        }

        .hero-title-reveal {
            animation: maskSlide 1.4s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }

        .fade-down { transform: translateY(-20px); transition: all 1.2s cubic-bezier(0.16, 1, 0.3, 1); }
        .fade-up { transform: translateY(30px); transition: all 1.2s cubic-bezier(0.16, 1, 0.3, 1); }
        ...
        .hero-bg-gradient {
            position: absolute;
            inset: 0;
            background: radial-gradient(circle at 50% 50%, rgba(59, 130, 246, 0.08) 0%, var(--enterprise-navy) 70%);
            z-index: 0;
        }

        /* --- Mobile Excellence & Typography --- */
        .responsive-title {
        ...
        .reveal-section { opacity: 0; transform: translateY(40px); transition: all 1.2s cubic-bezier(0.16, 1, 0.3, 1); }
        .reveal-section.active { opacity: 1; transform: translateY(0); }

        .stagger-card { opacity: 0; transform: translateY(20px); transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
        ...
        .glass-card:hover { border-color: var(--electric-blue); box-shadow: 0 0 40px rgba(59, 130, 246, 0.15); }

        .pulse-icon { animation: heartbeat 4s infinite ease-in-out; }
        @keyframes heartbeat {
            0%, 100% { transform: scale(1); filter: drop-shadow(0 0 0 transparent); }
            50% { transform: scale(1.1); filter: drop-shadow(0 0 15px var(--electric-blue)); }
        }
...
<body class="font-jakarta antialiased" x-data="{ mobileMenu: false }">
    ...
    <nav id="master-nav" class="fixed top-0 w-full z-[100] bg-[#0a0f1d]/80 backdrop-blur-3xl border-b border-white/5 initial-hidden fade-down">
        <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <span class="text-2xl font-black font-outfit uppercase text-white tracking-tighter">rooterin<span class="text-blue-500">.com</span></span>
            </div>
        ...
        <!-- Mobile Menu Overlay -->
        <div x-show="mobileMenu" x-cloak class="fixed inset-0 bg-[#0a0f1d] z-[110] p-8 flex flex-col" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
            <div class="flex items-center justify-between mb-16">
                <span class="text-2xl font-black font-outfit uppercase text-white">rooterin<span class="text-blue-500">.com</span></span>
                <button @click="mobileMenu = false" class="p-3 bg-white/5 rounded-2xl text-white">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                </button>
            </div>
            <div class="flex flex-col gap-10">
                <a @click="mobileMenu = false" href="#core" class="text-4xl font-black text-white uppercase tracking-tighter">Core Systems</a>
                <a @click="mobileMenu = false" href="#capabilities" class="text-4xl font-black text-white uppercase tracking-tighter">Capabilities</a>
                <a @click="mobileMenu = false" href="#solutions" class="text-4xl font-black text-white uppercase tracking-tighter">Solutions</a>
                <div class="h-px bg-white/10"></div>
                <a href="{{ route('login') }}" class="w-full py-5 bg-white text-slate-900 rounded-3xl font-black text-center shadow-2xl uppercase tracking-widest">Portal Login</a>
            </div>
        </div>
    </nav>
    ...
    <!-- Sections Flow -->
    <div class="relative z-10 space-y-24 md:space-y-48 pb-32">
        <!-- 1. CORE SYSTEMS -->
        <section id="core" class="scroll-mt-20 px-6">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-24 reveal-section">
                    <h2 class="text-4xl md:text-7xl font-black mb-6 uppercase tracking-tighter font-outfit text-white">Core Systems</h2>
        ...
        <!-- 2. CAPABILITIES -->
        <section id="capabilities" class="scroll-mt-20 px-6">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-24 reveal-section">
                    <h2 class="text-4xl md:text-7xl font-black mb-6 uppercase tracking-tighter font-outfit text-white">Capabilities</h2>
                    <p class="text-slate-500 font-bold uppercase tracking-[0.4em] text-[12px]">Advanced technical specs.</p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
                    <div class="stagger-card glass-card p-10 md:p-12 rounded-[48px] group">
        ...
        <!-- 3. SOLUTIONS -->
        <section id="solutions" class="scroll-mt-20 px-6">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-24 reveal-section">
                    <h2 class="text-4xl md:text-7xl font-black mb-6 uppercase tracking-tighter font-outfit text-white">Solutions</h2>
                    <p class="text-slate-500 font-bold uppercase tracking-[0.4em] text-[12px]">Bespoke excellence.</p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                    <div class="stagger-card glass-card p-10 md:p-16 rounded-[56px] group">
    ...
    <script>
        ...
        window.addEventListener('load', () => {
            ...
            setTimeout(() => { title.classList.add('hero-title-reveal'); title.style.opacity = 1; }, 500);
            setTimeout(() => nav.classList.add('visible'), 1200);
            setTimeout(() => { tag.classList.add('visible'); particles.classList.add('visible'); subtext.classList.add('visible'); }, 1600);
