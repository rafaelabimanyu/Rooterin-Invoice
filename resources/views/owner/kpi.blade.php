    ...
    <div class="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8 page-fade-in">
        <div>
            <div class="flex items-center gap-4 mb-4">
                <div class="w-14 h-14 rounded-2xl bg-slate-900 flex items-center justify-center text-white shadow-2xl shadow-slate-900/20 animate-float">
                    <i data-lucide="trending-up" class="w-8 h-8"></i>
                </div>
                <div>
                    <h1 class="text-4xl font-black text-slate-900 font-jakarta tracking-tight uppercase">Executive Intelligence</h1>
                    <p class="text-sm text-slate-500 font-bold uppercase tracking-[0.1em] mt-1">High-level business performance & growth analytics</p>
                </div>
            </div>
        </div>
            ...
            <div class="px-5 py-2.5 glass-panel rounded-2xl shadow-sm flex items-center gap-4 border-slate-200/50">
                <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-2 mt-1">
                        ...
                        <span class="text-[11px] font-bold text-slate-700">Live Database Feed</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12 page-fade-in" style="animation-delay: 100ms">
        ...
        <div class="glass-card p-8 group hover:-translate-y-2 hover:shadow-2xl transition-all duration-500 relative overflow-hidden border-indigo-500/10">
            <div class="flex items-center justify-between mb-8">
                ...
                <div class="p-3 bg-indigo-50 rounded-2xl text-indigo-600 group-hover:rotate-12 transition-transform duration-500">
                    <i data-lucide="banknote" class="w-6 h-6"></i>
                </div>
            </div>
            <h3 class="text-3xl font-black text-slate-900 font-jakarta tracking-tighter mb-4">Rp {{ number_format($currentMonthRevenue, 0, ',', '.') }}</h3>
            <div class="pt-6 border-t border-slate-50 flex items-center justify-between">
                <span class="text-[10px] text-slate-400 font-black uppercase tracking-widest">Growth vs Last Month</span>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-black {{ $revenueChange >= 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600' }} shadow-sm">
        ...
        <div class="glass-card p-8 group hover:-translate-y-2 hover:shadow-2xl transition-all duration-500 relative overflow-hidden border-amber-500/10">
            <div class="flex items-center justify-between mb-8">
                ...
                <div class="p-3 bg-amber-50 rounded-2xl text-amber-600 group-hover:rotate-12 transition-transform duration-500">
            ...
            <div class="pt-6 border-t border-slate-50 grid grid-cols-2 gap-4">
                <div class="flex flex-col">
                    ...
                    <span class="text-[12px] font-black text-slate-700">Rp {{ number_format($pendingUnpaid, 0, ',', '.') }}</span>
                </div>
        ...
        <div class="glass-card p-8 group hover:-translate-y-2 hover:shadow-2xl transition-all duration-500 relative overflow-hidden border-emerald-500/10">
            <div class="flex items-center justify-between mb-8">
                ...
                <div class="p-3 bg-emerald-50 rounded-2xl text-emerald-600 group-hover:rotate-12 transition-transform duration-500">
                    <i data-lucide="refresh-cw" class="w-6 h-6"></i>
                </div>
            </div>
            <h3 class="text-4xl font-black text-slate-900 font-jakarta tracking-tighter mb-4">{{ number_format($repeatRate, 1) }}%</h3>
        ...
        <div class="glass-card p-8 group hover:-translate-y-2 hover:shadow-2xl transition-all duration-500 relative overflow-hidden border-rose-500/10">
            <div class="flex items-center justify-between mb-8">
                ...
                <div class="p-3 bg-rose-50 rounded-2xl text-rose-600 group-hover:rotate-12 transition-transform duration-500">
                    <i data-lucide="crown" class="w-6 h-6"></i>
                </div>
            </div>
            @if($topClients->count() > 0)
                <h3 class="text-xl font-black text-slate-900 font-jakarta tracking-tight truncate mb-1" title="{{ $topClients[0]->nama_client }}">{{ $topClients[0]->nama_client }}</h3>
                ...
                <div class="mt-8 pt-6 border-t border-slate-50 flex items-center justify-between">
                    ...
                    <span class="text-[14px] font-black text-slate-900 tracking-tighter">Rp {{ number_format($topClients[0]->invoices_sum_total, 0, ',', '.') }}</span>
                </div>
            @else
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12 page-fade-in" style="animation-delay: 200ms">
        ...
        <div class="lg:col-span-2 glass-card p-10">
            <div class="flex items-center justify-between mb-10">
                <div>
                    <h3 class="font-black text-slate-900 font-jakarta uppercase tracking-tight text-xl">Revenue Vectors</h3>
                    <p class="text-[11px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">6-Month historical performance analysis</p>
                </div>
                <div class="flex items-center gap-3">
                    <div class="flex items-center gap-2 px-4 py-1.5 bg-indigo-50 rounded-full text-[10px] font-black text-indigo-600 uppercase tracking-widest">
        ...
        <div class="glass-card overflow-hidden flex flex-col group">
            <div class="p-10 border-b border-slate-100 bg-slate-50/30">
                <h3 class="font-black text-slate-900 font-jakarta uppercase tracking-tight text-xl">Operational Summary</h3>
            ...
            <div class="flex-1 p-10 flex flex-col justify-center space-y-10 relative">
                <div class="flex items-center justify-between group/item">
                    <div class="flex items-center gap-5">
                        <div class="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 group-hover/item:scale-110 transition-transform">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-slate-900 font-jakarta">{{ $monthlyPerformance['created'] }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between group/item">
                    <div class="flex items-center gap-5">
                        <div class="w-14 h-14 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600 group-hover/item:scale-110 transition-transform">
                            <i data-lucide="check-circle" class="w-6 h-6"></i>
                        </div>
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Settled Assets</p>
                            <p class="text-2xl font-black text-slate-900 font-jakarta">{{ $monthlyPerformance['paid'] }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between group/item">
                    <div class="flex items-center gap-5">
                        <div class="w-14 h-14 rounded-2xl bg-rose-50 flex items-center justify-center text-rose-600 group-hover/item:scale-110 transition-transform">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-black text-slate-900 font-jakarta">{{ $monthlyPerformance['overdue'] }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($topClients as $index => $client)
                            <tr class="table-row-premium">
                                <td class="px-10 py-6">
                                    <div class="flex items-center gap-5">
                                        <span class="w-8 h-8 rounded-lg bg-slate-900 flex items-center justify-center text-[10px] font-black text-white shadow-lg">{{ $index + 1 }}</span>
                                        <div class="flex flex-col">
                                            <span class="text-[14px] font-black text-slate-900 tracking-tight">{{ $client->nama_client }}</span>
                                            <span class="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">{{ $client->nama_perusahaan }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-10 py-6">
                                    <div class="inline-flex items-center gap-2 px-3 py-1 bg-slate-100 rounded-full text-[11px] font-bold text-slate-600">
                                        <i data-lucide="file-text" class="w-3.5 h-3.5"></i>
                                        {{ $client->invoices_count }} Units
                                    </div>
                                </td>
                                <td class="px-10 py-6 text-right">
                                    <span class="text-[15px] font-black text-slate-900 tracking-tighter">Rp {{ number_format($client->invoices_sum_total, 0, ',', '.') }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="p-6">
                <div class="space-y-4">
                    @forelse($recentLargePayments as $payment)
                        <div class="flex items-center justify-between p-5 rounded-2xl bg-slate-50/50 hover:bg-white hover:shadow-xl hover:-translate-x-1 transition-all duration-300 border border-transparent hover:border-slate-200/50 group">
                            <div class="flex items-center gap-5">
                                <div class="w-14 h-14 rounded-2xl bg-white flex flex-col items-center justify-center border border-slate-100 shadow-sm group-hover:bg-slate-900 transition-colors duration-500">
                                    <span class="text-[10px] font-black text-slate-400 uppercase leading-none group-hover:text-slate-500">{{ $payment->payment_date->format('M') }}</span>
                                    <span class="text-lg font-black text-slate-900 leading-none mt-1 group-hover:text-white">{{ $payment->payment_date->format('d') }}</span>
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-[14px] font-black text-slate-900 tracking-tight group-hover:text-indigo-600 transition-colors">{{ $payment->invoice->client->nama_client }}</span>
                            ...
                            <div class="flex flex-col text-right">
                                <span class="text-[16px] font-black text-emerald-600 tracking-tighter">Rp {{ number_format($payment->amount, 0, ',', '.') }}</span>
