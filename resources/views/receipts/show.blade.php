<x-app-layout :title="app()->getLocale() == 'en' ? 'Receipt & Payment Management' : 'Manajemen Kwitansi & Pembayaran'">
    <div class="mb-8 md:mb-12 flex flex-col lg:flex-row lg:items-end justify-between gap-6 px-4 md:px-0">
        ...
        <div class="flex flex-col sm:flex-row flex-wrap sm:flex-nowrap items-stretch sm:items-center gap-3 md:gap-4 w-full sm:w-auto">
            ...
            @if($receipt->status !== 'invoiced')
                <a href="{{ route('receipts.edit', $receipt->id) }}" class="hidden md:inline-flex px-6 py-3 bg-amber-500 text-slate-950 rounded-xl text-sm font-bold hover:bg-amber-600 transition-all shadow-lg hover:shadow-amber-500/20 items-center justify-center gap-2 active:scale-95">
            ...
            <div class="flex flex-col md:flex-row justify-between gap-12">
                <div class="w-full md:flex-1 space-y-8">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-bold text-slate-800 leading-relaxed">{{ $receipt->invoice->technician_names }}</p>
                    </div>
                    @endif
                </div>
    ...
    <div class="md:hidden fixed bottom-0 left-0 right-0 z-50 pointer-events-none p-4 flex justify-center">
        <div class="pointer-events-auto w-full bg-[#0F2A44] border border-slate-800 p-4 rounded-2xl flex gap-3 shadow-2xl items-center justify-between">
            <div class="flex items-center gap-2">
                <x-badge :status="$receipt->status" class="scale-90" />
            </div>
            <div class="flex gap-2">
                @if($receipt->status !== 'invoiced')
                    <a href="{{ route('receipts.edit', $receipt->id) }}" class="px-4 py-2.5 bg-amber-500 text-slate-950 rounded-xl text-xs font-bold transition-all active:scale-95 flex items-center gap-1">
                        <i data-lucide="edit-3" class="w-3.5 h-3.5"></i>
                        <span>{{ __('ui.edit') ?? 'Edit' }}</span>
                    </a>
                @endif
                <button type="button" @click="$dispatch('download-pdf', { url: '{{ route('receipts.pdf', $receipt) }}', filename: 'Receipt-{{ $receipt->receipt_number }}.pdf' })" class="px-4 py-2.5 bg-white text-[#0F2A44] rounded-xl text-xs font-bold transition-all active:scale-95 flex items-center gap-1">
