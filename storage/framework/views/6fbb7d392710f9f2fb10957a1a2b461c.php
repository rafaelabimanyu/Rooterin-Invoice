...
<?php $component = App\View\Components\AppLayout::resolve(['title' => 'Rooterin AI Assistant'] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
                ...
                >
                    <div class="flex flex-col h-full overflow-hidden">
                        ...
                        <div class="p-5 border-b border-slate-100 flex items-center justify-between shrink-0">
                            <div>
                                <h3 class="text-xs font-black text-slate-800 uppercase tracking-wider font-jakarta"><?php echo e(app()->getLocale() == 'en' ? 'Chat History' : 'Riwayat Obrolan'); ?></h3>
                            ...
                            <div class="flex items-center gap-1.5">
                                <button @click="newChat(); showDrawer = false" 
                                    ...
                                    title="<?php echo e(app()->getLocale() == 'en' ? 'Start New Chat' : 'Mulai Obrolan Baru'); ?>">
                                ...
                                <button @click="showDrawer = false" 
                                    ...
                                    title="<?php echo e(app()->getLocale() == 'en' ? 'Close Menu' : 'Tutup Menu'); ?>">
                        ...
                        <div x-ref="drawerHistoryContainer" class="flex-1 overflow-y-auto p-4 space-y-2.5 chat-scroll">
                            ...
                            <template x-if="sessions.length === 0">
                                <div class="text-center py-12 px-4">
                                    ...
                                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest"><?php echo e(app()->getLocale() == 'en' ? 'No history yet' : 'Belum ada riwayat'); ?></p>
                                </div>
                            </template>
                        </div>
                    </div>
        ...
        <div class="hidden lg:flex lg:w-80 bg-white lg:border-r border-slate-200/80 flex-col justify-between shrink-0 lg:h-full">
            <div class="flex flex-col h-full overflow-hidden">
                ...
                <div class="p-6 border-b border-slate-100 flex items-center justify-between shrink-0">
                    <div>
                        <h3 class="text-sm font-black text-slate-800 uppercase tracking-wider font-jakarta"><?php echo e(app()->getLocale() == 'en' ? 'Chat History' : 'Riwayat Obrolan'); ?></h3>
                ...
                <div x-ref="historyContainer" class="flex-1 overflow-y-auto p-4 space-y-2.5 chat-scroll">
                            ...
                            <template x-if="editingSessionId !== sess.session_id">
                                <div class="flex items-center w-full justify-between p-3.5 gap-2 pr-1">
                                    ...
                                    <div class="hidden group-hover:flex items-center gap-1.5 shrink-0 px-2 bg-gradient-to-l from-slate-50 via-slate-50/95 to-transparent absolute right-2 top-1/2 -translate-y-1/2 py-1.5 pl-4 rounded-lg"
                                         :class="currentSessionId === sess.session_id ? 'from-indigo-50 via-indigo-50/95' : ''">
                                        <!-- Rename Button -->
                                        <button @click.stop="startRename(sess)" class="p-1 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-md transition-all active:scale-90" title="<?php echo e(app()->getLocale() == 'en' ? 'Rename' : 'Ubah Nama'); ?>">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125"></path>
                                            </svg>
                                        </button>
                                        <!-- Delete Button -->
                                        <button @click.stop="deleteSession(sess.session_id)" class="p-1 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-md transition-all active:scale-90" title="<?php echo e(app()->getLocale() == 'en' ? 'Delete' : 'Hapus'); ?>">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"></path>
                                            </svg>
                                        </button>
                            ...
                            <template x-if="editingSessionId === sess.session_id">
                                <div class="flex items-center gap-1.5 p-2 w-full">
                                    <input x-model="editingTitle" 
                                        ...
                                        x-ref="renameInput"
                                    >
                                    <button @click="saveRename(sess.session_id)" class="p-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 rounded-xl transition-all active:scale-95" title="<?php echo e(app()->getLocale() == 'en' ? 'Save' : 'Simpan'); ?>">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"></path>
                                        </svg>
                                    </button>
                                    <button @click="cancelRename()" class="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl transition-all active:scale-95" title="<?php echo e(app()->getLocale() == 'en' ? 'Cancel' : 'Batal'); ?>">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path>
                                        </svg>
                                    </button>
                    ...
                    <template x-if="sessions.length === 0">
                        <div class="text-center py-12 px-4">
                            ...
                            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest"><?php echo e(app()->getLocale() == 'en' ? 'No history yet' : 'Belum ada riwayat'); ?></p>
                            <p class="text-[10px] text-slate-400 mt-1 leading-relaxed"><?php echo e(app()->getLocale() == 'en' ? 'Start sending messages to save your conversation log.' : 'Mulai kirim pesan untuk menyimpan log percakapan Anda.'); ?></p>
                        </div>
                    </template>
                </div>
            </div>
            ...
            <div class="p-6 bg-slate-50/50 border-t border-slate-100 shrink-0 hidden lg:block">
                <div class="flex items-center gap-2.5 text-slate-700 font-bold text-xs">
                    ...
                    <span><?php echo e(app()->getLocale() == 'en' ? 'System Secure & Encrypted' : 'Sistem Aman & Terenkripsi'); ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 flex flex-col h-full bg-slate-50/20 min-w-0">
            ...
            <div class="px-4 lg:px-8 py-4 lg:py-5 bg-white border-b border-slate-200/80 flex items-center justify-between gap-3 shrink-0">
                <div class="flex items-center gap-3 lg:gap-4 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span class="text-[8px] lg:text-[9px] text-slate-400 font-bold uppercase tracking-wider truncate"><?php echo e(app()->getLocale() == 'en' ? 'Senior Financial Consultant' : 'Konsultan Finansial Senior'); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <button @click="showDrawer = true; $nextTick(() => { if (typeof lucide !== 'undefined') { lucide.createIcons(); } })" 
                        ...
                        title="<?php echo e(app()->getLocale() == 'en' ? 'Open History' : 'Buka Riwayat'); ?>">
                        <i data-lucide="history" class="w-4 h-4 lg:w-4.5 lg:h-4.5"></i>
                    </button>
                    <a href="<?php echo e(route('dashboard')); ?>" class="p-2 lg:p-2.5 bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-700 rounded-xl transition-colors border border-slate-200" title="<?php echo e(app()->getLocale() == 'en' ? 'Back to Dashboard' : 'Kembali ke Dashboard'); ?>">
                ...
                <template x-if="messages.length <= 1 && !loading">
                    <div class="max-w-2xl mx-auto py-4 lg:py-8 space-y-6 lg:space-y-8 h-auto px-2 lg:px-0">
                        <div class="text-center space-y-2 lg:space-y-3 h-auto py-1">
                            ...
                            <h3 class="text-base lg:text-lg font-black text-slate-900 font-jakarta uppercase tracking-tight"><?php echo e(app()->getLocale() == 'en' ? 'Welcome to Rooterin AI 2.0' : 'Selamat Datang di Rooterin AI 2.0'); ?></h3>
                            <p class="text-[11px] lg:text-xs text-slate-500 font-medium leading-relaxed max-w-md mx-auto px-4 lg:px-0 h-auto pb-1">
                                <?php echo e(app()->getLocale() == 'en' ? 'Ask questions about cash flow, receivable analysis, client status, or ask the assistant to navigate to a specific page directly.' : 'Ajukan pertanyaan tentang cashflow, analisis penagihan piutang, status klien, atau minta asisten untuk membuka halaman tertentu secara langsung.'); ?>
                        ...
                        <div class="space-y-3 w-full max-w-full overflow-hidden">
                            <p class="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-center px-4"><?php echo e(app()->getLocale() == 'en' ? 'Recommended Popular Questions' : 'Rekomendasi Pertanyaan Populer'); ?></p>
                            ...
                            <div class="flex overflow-x-auto snap-x snap-mandatory gap-3 pb-2 no-scrollbar lg:grid lg:grid-cols-2 lg:overflow-x-visible lg:gap-4 lg:pb-0 px-4 lg:px-0">
                                ...
                                <button @click="sendSuggestion('<?php echo e(app()->getLocale() == 'en' ? 'This month\'s cash flow analysis' : 'Analisis arus kas bulan ini'); ?>')" 
                                    ...
                                    <div class="space-y-0.5 min-w-0">
                                        <span class="text-xs font-bold text-slate-800 group-hover:text-indigo-950 transition-colors block truncate"><?php echo e(app()->getLocale() == 'en' ? 'Cash Flow Analysis' : 'Analisis Arus Kas'); ?></span>
                                        <p class="text-[10px] text-slate-400 font-medium truncate"><?php echo e(app()->getLocale() == 'en' ? 'Compare paid & overdue this month.' : 'Bandingkan lunas & overdue bulan ini.'); ?></p>
                                    </div>
                                </button>

                                <button @click="sendSuggestion('<?php echo e(app()->getLocale() == 'en' ? 'Who are the most overdue clients?' : 'Siapa klien yang paling sering menunggak?'); ?>')" 
                                    class="min-w-[85%] snap-center lg:min-w-0 lg:w-auto shrink-0 lg:shrink text-left p-4 bg-white hover:bg-indigo-50/30 border border-slate-200 hover:border-indigo-300 rounded-2xl transition-all flex items-center gap-3.5 group shadow-sm hover:shadow active:scale-98">
                                    ...
                                    <div class="space-y-0.5 min-w-0">
                                        <span class="text-xs font-bold text-slate-800 group-hover:text-indigo-950 transition-colors block truncate"><?php echo e(app()->getLocale() == 'en' ? 'Overdue Clients' : 'Klien Terlambat Bayar'); ?></span>
                                        <p class="text-[10px] text-slate-400 font-medium truncate"><?php echo e(app()->getLocale() == 'en' ? 'Detect clients with accumulated receivables.' : 'Deteksi klien dengan piutang menumpuk.'); ?></p>
                                    </div>
                                </button>

                                <button @click="sendSuggestion('<?php echo e(app()->getLocale() == 'en' ? 'Create summary report for owner meeting' : 'Buat laporan ringkas untuk meeting owner'); ?>')" 
                                    class="min-w-[85%] snap-center lg:min-w-0 lg:w-auto shrink-0 lg:shrink text-left p-4 bg-white hover:bg-indigo-50/30 border border-slate-200 hover:border-indigo-300 rounded-2xl transition-all flex items-center gap-3.5 group shadow-sm hover:shadow active:scale-98">
                                    ...
                                    <div class="space-y-0.5 min-w-0">
                                        <span class="text-xs font-bold text-slate-800 group-hover:text-indigo-950 transition-colors block truncate"><?php echo e(app()->getLocale() == 'en' ? 'Summary Executive Meeting' : 'Ringkasan Executive Meeting'); ?></span>
                                        <p class="text-[10px] text-slate-400 font-medium truncate"><?php echo e(app()->getLocale() == 'en' ? 'Summary of latest operational KPIs.' : 'Ringkasan KPI operasional terkini.'); ?></p>
                                    </div>
                                </button>

                                <button @click="sendSuggestion('<?php echo e(app()->getLocale() == 'en' ? 'Revenue projection for the next 3 months' : 'Prediksi pendapatan 3 bulan ke depan'); ?>')" 
                                    class="min-w-[85%] snap-center lg:min-w-0 lg:w-auto shrink-0 lg:shrink text-left p-4 bg-white hover:bg-indigo-50/30 border border-slate-200 hover:border-indigo-300 rounded-2xl transition-all flex items-center gap-3.5 group shadow-sm hover:shadow active:scale-98">
                                    ...
                                    <div class="space-y-0.5 min-w-0">
                                        <span class="text-xs font-bold text-slate-800 group-hover:text-indigo-950 transition-colors block truncate"><?php echo e(app()->getLocale() == 'en' ? 'Revenue Projection' : 'Prediksi Pendapatan'); ?></span>
                                        <p class="text-[10px] text-slate-400 font-medium truncate"><?php echo e(app()->getLocale() == 'en' ? 'Cash projection based on 3-month trend.' : 'Proyeksi kas berdasarkan tren 3 bulan.'); ?></p>
                                    </div>
                                </button>

                                <button @click="sendSuggestion('<?php echo e(app()->getLocale() == 'en' ? 'How to improve invoice collectibility?' : 'Bagaimana cara meningkatkan kolektibilitas invoice?'); ?>')" 
                                    ...
                                    <div class="space-y-0.5 min-w-0">
                                        <span class="text-xs font-bold text-slate-800 group-hover:text-indigo-950 transition-colors block truncate"><?php echo e(app()->getLocale() == 'en' ? 'Optimize Invoice Collectibility' : 'Optimalkan Kolektibilitas Invoice'); ?></span>
                                        <p class="text-[10px] text-slate-400 font-medium truncate"><?php echo e(app()->getLocale() == 'en' ? 'Tactical advice to reduce overdue receivables and maintain cash rotation.' : 'Saran taktis untuk mengurangi piutang overdue dan menjaga perputaran kas.'); ?></p>
                                    </div>
                                </button>
                            </div>
                        </div>
                        ...
                        <template x-if="msg.sender === 'ai' && idx > 0">
                            <div class="flex items-center gap-3 mt-1.5 ml-3">
                                ...
                                >
                                    ...
                                    <span x-text="copiedIndex === idx ? '<?php echo e(app()->getLocale() == 'en' ? 'Copied!' : 'Tersalin!'); ?>' : '<?php echo e(app()->getLocale() == 'en' ? 'Copy Text' : 'Salin Teks'); ?>'"></span>
                                </button>
                            </div>
                        </template>
            ...
            >
                <div class="flex items-center gap-2.5 text-rose-700 text-[10px] font-bold uppercase tracking-wider">
                    ...
                    <span><?php echo e(app()->getLocale() == 'en' ? 'Listening to voice...' : 'Mendengarkan suara...'); ?></span>
                </div>
                <!-- Audio Wave Bars -->
            ...
            <form @submit.prevent="sendMessage()" class="p-3 sm:p-4 lg:p-6 bg-white border-t border-slate-200 flex items-center gap-2 lg:gap-4 shrink-0">
                <input x-model="input" 
                    type="text" 
                    placeholder="<?php echo e(app()->getLocale() == 'en' ? 'Ask financial analysis, overdue bills...' : 'Tanyakan analisis keuangan, tagihan overdue...'); ?>" 
                ...
                <button type="button" 
                    ...
                    :disabled="loading"
                    title="<?php echo e(app()->getLocale() == 'en' ? 'Voice input (Speech to Text)' : 'Input suara (Speech to Text)'); ?>">
    ...
    <script>
        const registerAiChat = () => {
            Alpine.data('aiChat', () => ({
                ...
                sessions: <?php echo \Illuminate\Support\Js::from($sessions)->toHtml() ?>,
                ...
                routeMap: {
                    ...
                    'invoices.index': '<?php echo e(route('invoices.index')); ?>',
                    'invoices.create': '<?php echo e(route('invoices.create')); ?>',
                    'clients.index': '<?php echo e(route('clients.index')); ?>',
                    'clients.create': '<?php echo e(route('clients.create')); ?>',
                    'receipts.index': '<?php echo e(route('receipts.index')); ?>',
                    'receipts.create': '<?php echo e(route('receipts.create')); ?>',
                    'settings.index': '<?php echo e(route('settings.index')); ?>',
                    'profile.edit': '<?php echo e(route('profile.edit')); ?>',
                    'reports.index': '<?php echo e(route('reports.index')); ?>',
                    'chronos.index': '<?php echo e(route('chronos.index')); ?>'
                },
                routeLabels: {
                    'dashboard': '<?php echo e(app()->getLocale() == "en" ? "👉 Open Main Dashboard" : "👉 Buka Dashboard Utama"); ?>',
                    'invoices.index': '<?php echo e(app()->getLocale() == "en" ? "👉 View Invoices List" : "👉 Lihat Daftar Invoice"); ?>',
                    'invoices.create': '<?php echo e(app()->getLocale() == "en" ? "👉 Create New Invoice" : "👉 Buat Invoice Baru"); ?>',
                    'clients.index': '<?php echo e(app()->getLocale() == "en" ? "👉 View Clients List" : "👉 Lihat Daftar Klien"); ?>',
                    'clients.create': '<?php echo e(app()->getLocale() == "en" ? "👉 Add New Client" : "👉 Tambah Klien Baru"); ?>',
                    'receipts.index': '<?php echo e(app()->getLocale() == "en" ? "👉 View Receipts List" : "👉 Lihat Daftar Kuitansi"); ?>',
                    'receipts.create': '<?php echo e(app()->getLocale() == "en" ? "👉 Create New Receipt" : "👉 Buat Kuitansi Baru"); ?>',
                    'settings.index': '<?php echo e(app()->getLocale() == "en" ? "👉 Open Settings" : "👉 Buka Pengaturan"); ?>',
                    'profile.edit': '<?php echo e(app()->getLocale() == "en" ? "👉 Edit My Profile" : "👉 Edit Profil Saya"); ?>',
                    'reports.index': '<?php echo e(app()->getLocale() == "en" ? "👉 Open Financial Reports" : "👉 Buka Laporan Keuangan"); ?>',
                    'chronos.index': '<?php echo e(app()->getLocale() == "en" ? "👉 Open Billing Calendar (Chronos)" : "👉 Buka Kalender Billing (Chronos)"); ?>'
                },
                initChat() {
                    this.messages = [
                        { sender: 'ai', text: '<?php echo e(app()->getLocale() == "en" ? "Hello! I am your Virtual Senior Financial Consultant & Business Analyst. Is there anything I can help you with regarding cash flow analysis, overdue clients, financial forecasts, or system navigation today?" : "Halo! Saya Senior Financial Consultant & Business Analyst Virtual Anda. Ada yang bisa saya bantu terkait analisis arus kas, klien overdue, prediksi keuangan, atau navigasi sistem hari ini?"); ?>' }
                    ];
                    this.initSpeech();
                ...
                initSpeech() {
                    ...
                    this.recognition.lang = '<?php echo e(app()->getLocale() == "en" ? "en-US" : "id-ID"); ?>';
                ...
                loadSession(sessionId) {
                    ...
                    fetch('/ai-assistant/session/' + sessionId)
                        ...
                        .catch(err => {
                            this.loading = false;
                            this.messages.push({ sender: 'ai', text: '<?php echo e(app()->getLocale() == "en" ? "Sorry, failed to load chat history: " : "Maaf, gagal memuat riwayat obrolan: "); ?>' + err.message });
                ...
                sendMessage() {
                    ...
                    fetch('<?php echo e(route('ai-assistant.chat')); ?>', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            this.messages.push({ sender: 'ai', text: '<?php echo e(app()->getLocale() == "en" ? "Sorry, an error occurred while processing your request." : "Maaf, terjadi kesalahan saat memproses permintaan Anda."); ?>' });
                        }
                        this.$nextTick(() => {
                            this.scrollToBottom();
                        });
                    ...
                    .catch(err => {
                        this.loading = false;
                        this.messages.push({ sender: 'ai', text: '<?php echo e(app()->getLocale() == "en" ? "Sorry, failed to process. " : "Maaf, gagal memproses. "); ?>' + err.message });
                        this.$nextTick(() => {
                            this.scrollToBottom();
                        });
                    });
                ...
                processResponse(reply) {
                    ...
                    if (routeName && this.routeMap[routeName]) {
                        ...
                        msg.navigateLabel = this.routeLabels[routeName] || '<?php echo e(app()->getLocale() == "en" ? "👉 Open Page" : "👉 Buka Halaman"); ?>';
                    }
                    
                    this.messages.push(msg);
                },
                refreshSessionsList() {
                    fetch('<?php echo e(route('ai-assistant.sessions-list')); ?>')
                        .then(res => res.json())
                        .then(data => {
                            if (data.success) {
                ...
                deleteSession(sessionId) {
                    if (!confirm('<?php echo e(app()->getLocale() == "en" ? "Are you sure you want to delete this chat session?" : "Apakah Anda yakin ingin menghapus sesi obrolan ini?"); ?>')) {
                        return;
                    }
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\Rooterin-Invoice\resources\views/ai-assistant/index.blade.php ENDPATH**/ ?>