...
<div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-8 page-fade-in">
    <div>
        ...
        <p class="text-sm text-slate-500 font-medium tracking-tight">
            <?php echo e(__('Billing Calendar & Operational Workflows')); ?>
...
<div class="grid grid-cols-1 md:flex md:flex-wrap md:items-center gap-6 p-6 glass-card border-slate-100 shadow-xl shadow-indigo-500/5 bg-white/70 backdrop-blur-md rounded-3xl select-none relative z-30">
    ...
    <div class="relative flex-1 min-w-[200px]" x-data="{ open: false }" @click.outside="open = false" :class="{ 'z-30': open, 'z-10': !open }">
        ...
        <button @click="open = !open" type="button" class="premium-input w-full flex items-center justify-between text-left bg-white/70 border border-slate-200/80 rounded-2xl px-4 py-3 text-xs font-bold text-slate-700 shadow-sm transition-all hover:bg-slate-50/50">
            <div class="flex items-center gap-2 truncate">
                ...
                <span class="truncate">
                    <?php echo e($clientId ? ($clients->firstWhere('id', $clientId)->nama_client ?? __('All Clients')) : __('All Clients')); ?>
        ...
        >
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $clients; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $client): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                <button wire:click="$set('clientId', <?php echo e($client->id); ?>);" @click="open = false" type="button" class="w-full text-left px-3.5 py-2.5 text-xs font-bold rounded-xl transition-colors
                    <?php echo e($clientId == $client->id ? 'bg-indigo-50 text-indigo-755' : 'text-slate-655 hover:bg-slate-50 hover:text-indigo-650'); ?>"
                >
                    <?php echo e($client->nama_client); ?>
    ...
    <div class="flex-1 min-w-[250px] md:flex-initial">
        ...
        <div class="flex items-center gap-1.5 overflow-x-auto scrollbar-none py-0.5">
            <button wire:click="$set('status', '')" type="button" 
                ...
                <?php echo e($status === '' ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-600/15' : 'bg-white/70 text-slate-600 border-slate-200/60 hover:bg-slate-50'); ?>"
            >
                <?php echo e(__('All')); ?>
            ...
            <button wire:click="$set('status', 'draft')" type="button" 
                class="px-3.5 py-2.5 rounded-xl text-xs font-bold border transition-all duration-200 active:scale-95
                <?php echo e($status === 'draft' ? 'bg-amber-500 text-white border-amber-500 shadow-md shadow-amber-500/15' : 'bg-white/70 text-slate-600 border-slate-200/60 hover:bg-slate-50'); ?>"
            >
                <?php echo e(__('Draft')); ?>
            ...
            <button wire:click="$set('status', 'paid')" type="button" 
                class="px-3.5 py-2.5 rounded-xl text-xs font-bold border transition-all duration-200 active:scale-95
                <?php echo e($status === 'paid' ? 'bg-emerald-550 text-white border-emerald-500 shadow-md shadow-emerald-500/15' : 'bg-white/70 text-slate-600 border-slate-200/60 hover:bg-slate-50'); ?>"
            >
                <?php echo e(__('Paid')); ?>
            ...
            <button wire:click="$set('status', 'overdue')" type="button" 
                class="px-3.5 py-2.5 rounded-xl text-xs font-bold border transition-all duration-200 active:scale-95
                <?php echo e($status === 'overdue' ? 'bg-rose-550 text-white border-rose-500 shadow-md shadow-rose-500/15' : 'bg-white/70 text-slate-600 border-slate-200/60 hover:bg-slate-50'); ?>"
            >
                <?php echo e(__('Overdue')); ?>
            ...
            <button wire:click="$set('status', 'sent')" type="button" 
                class="px-3.5 py-2.5 rounded-xl text-xs font-bold border transition-all duration-200 active:scale-95
                <?php echo e($status === 'sent' ? 'bg-blue-500 text-white border-blue-500 shadow-md shadow-blue-500/15' : 'bg-white/70 text-slate-600 border-slate-200/60 hover:bg-slate-50'); ?>"
            >
                <?php echo e(__('Sent')); ?>
    ...
    <div class="relative flex-1 min-w-[200px]" x-data="{ open: false }" @click.outside="open = false" :class="{ 'z-20': open, 'z-10': !open }">
        <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">
            <?php echo e(__('Responsible Staff')); ?>
        ...
        <button @click="open = !open" type="button" class="premium-input w-full flex items-center justify-between text-left bg-white/70 border border-slate-200/80 rounded-2xl px-4 py-3 text-xs font-bold text-slate-700 shadow-sm transition-all hover:bg-slate-50/50">
            <div class="flex items-center gap-2 truncate">
                ...
                <span class="truncate">
                    <?php echo e($staffId ? ($staffs->firstWhere('id', $staffId)->name ?? __('All Staff')) : __('All Staff')); ?>
        ...
        >
            <button wire:click="$set('staffId', '');" @click="open = false" type="button" class="w-full text-left px-3.5 py-2.5 text-xs font-bold rounded-xl text-slate-655 hover:bg-slate-50 hover:text-indigo-650 transition-colors">
                <?php echo e(__('All Staff')); ?>

            </button>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $staffs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $staff): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                <button wire:click="$set('staffId', <?php echo e($staff->id); ?>);" @click="open = false" type="button" class="w-full text-left px-3.5 py-2.5 text-xs font-bold rounded-xl transition-colors
                    <?php echo e($staffId == $staff->id ? 'bg-indigo-50 text-indigo-755' : 'text-slate-655 hover:bg-slate-50 hover:text-indigo-650'); ?>"
                >
    ...
    <div class="flex items-end h-full pt-6 md:pt-0">
        <button wire:click="$set('clientId', ''); $set('status', ''); $set('staffId', '')" 
            ...
            title="<?php echo e(__('Reset Filters')); ?>"
        >
...
<?php /**PATH C:\laragon\www\Rooterin-Invoice\resources\views/chronos/components/header-filter.blade.php ENDPATH**/ ?>