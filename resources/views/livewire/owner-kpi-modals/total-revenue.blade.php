<div class="space-y-6 animate-fade-in">
    ...
    <div class="p-6 bg-indigo-600 rounded-2xl text-white shadow-xl shadow-indigo-600/10 flex justify-between items-center">
        <div>
            <span class="px-2.5 py-0.5 bg-white/20 rounded-full text-[8px] font-black uppercase tracking-widest">{{ app()->getLocale() == 'en' ? 'Lifetime Revenue' : 'Total Pendapatan Terkumpul' }}</span>
            <h3 class="text-2xl font-black mt-3 font-jakarta tracking-tight">Rp {{ number_format($totalRevenue, 0, ',', '.') }}</h3>
            <p class="text-[9.5px] text-indigo-200 font-medium mt-2">{{ app()->getLocale() == 'en' ? 'Gross revenue from paid invoices' : 'Pendapatan kotor dari seluruh invoice yang lunas' }}</p>
        </div>
