<x-app-layout :title="app()->getLocale() == 'en' ? 'Partnership Invoice Details' : 'Rincian Invoice Kemitraan'">
    <!-- Top Action Header -->
    <div class="mb-8 md:mb-12 flex flex-col lg:flex-row lg:items-end justify-between gap-6 px-4 md:px-0">
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 overflow-hidden">
                <a href="{{ route('contract-invoices.index') }}" class="hover:text-gold-600 transition-colors shrink-0">{{ app()->getLocale() == 'en' ? 'Partnership Invoices' : 'Invoice Kemitraan' }}</a>
            ...
            <p class="text-sm text-slate-500 mt-1 truncate">{{ app()->getLocale() == 'en' ? 'Manage partnership billing for ' : 'Kelola kemitraan tagihan untuk ' }}{{ optional($invoice->client)->nama_client ?? 'Klien Tidak Ditemukan' }}.</p>
        </div>
        <div class="flex flex-col sm:flex-row flex-wrap sm:flex-nowrap items-slate-900 sm:items-center gap-3 md:gap-4 w-full sm:w-auto">
            ...
            <a href="{{ route('contract-invoices.edit', $invoice->id) }}" class="hidden md:inline-flex px-6 py-3 bg-amber-500 text-slate-950 rounded-xl text-sm font-bold hover:bg-amber-600 transition-all shadow-lg hover:shadow-amber-500/20 items-center justify-center gap-2 active:scale-95">
                <i data-lucide="edit-3" class="w-4 h-4"></i>
                <span class="whitespace-nowrap">{{ __('ui.edit') ?? 'Edit' }}</span>
            </a>
            <button type="button" @click="$dispatch('download-pdf', { url: '{{ route('contract-invoices.pdf', $invoice) }}', filename: 'Invoice-{{ $invoice->invoice_number }}.pdf' })" title="Download PDF" class="w-full sm:w-auto px-6 py-3 bg-[#0F2A44] text-white rounded-xl text-sm font-bold hover:bg-slate-800 transition-all shadow-lg hover:shadow-[#0F2A44]/30 flex items-center justify-center gap-2 active:scale-95">
    ...
    <div class="bg-white rounded-[24px] md:rounded-[32px] border border-slate-200/60 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] overflow-hidden max-w-5xl mx-auto mb-20 relative w-full">
        ...
        <div class="p-6 md:p-16 border-b border-slate-100 relative">
            <div class="flex flex-col md:flex-row justify-between items-start gap-10 md:gap-12">
                ...
                <div class="w-full md:w-auto text-left md:text-right">
                    <h2 class="text-xs font-bold text-slate-400 uppercase tracking-[0.3em] mb-4 md:mb-6">{{ app()->getLocale() == 'en' ? 'Partnership Invoice' : 'Faktur Kemitraan' }}</h2>
                    <div class="space-y-1">
                        <p class="text-2xl font-black text-slate-900 font-outfit">{{ $invoice->invoice_number }}</p>
                        <p class="text-xs font-bold text-slate-500">{{ app()->getLocale() == 'en' ? 'Issued' : 'Dibuat' }}: {{ $invoice->tanggal_invoice ? $invoice->tanggal_invoice->format(\App\Models\Setting::get('date_format', 'd M Y')) : '-' }}</p>
                        @if($invoice->periode_kontrak)
                        <div class="inline-block mt-2 px-3 py-1 bg-amber-50 text-amber-700 border border-amber-100 rounded text-[10px] font-bold uppercase tracking-widest">
                            {{ __('ui.contract_period') }}: {{ $invoice->periode_kontrak }}
                        </div>
                        @endif
                        @if($invoice->due_date)
                        <br>
                        <div class="inline-block mt-2 px-3 py-1 bg-rose-50 text-rose-600 border border-rose-100 rounded text-[10px] font-bold uppercase tracking-widest">
                            {{ app()->getLocale() == 'en' ? 'Due' : 'Tempo' }}: {{ $invoice->due_date->format(\App\Models\Setting::get('date_format', 'd M Y')) }}
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="flex flex-col md:flex-row p-6 md:p-16 gap-10 md:gap-16 bg-slate-50/30 border-b border-slate-100">
            ...
            <div class="w-full md:w-auto text-left md:text-right">
                <div class="p-6 md:p-8 bg-white rounded-xl border border-slate-200 shadow-sm w-full">
                    ...
                    <p class="text-2xl md:text-3xl font-black text-slate-900 font-outfit">{{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($invoice->total, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>

        <!-- Line Items -->
        <div class="w-full border-b border-slate-100">
            <!-- Desktop Table -->
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($invoice->items as $item)
                        <tr>
                            ...
                            <td class="py-6 text-center text-[13px] font-medium text-slate-600">{{ number_format($item->qty, 2) }}</td>
                            ...
                            <td class="py-6 text-right px-16 text-[13px] font-black text-slate-900">{{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($item->qty * $item->harga, 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div class="w-full">
                            ...
                            <p class="text-xs font-medium text-slate-600">
                                {{ number_format($item->qty, 2) }} Unit &times; {{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($item->harga, 0, ',', '.') }}
                            </p>
                        </div>
                        <div class="w-full bg-slate-50 p-4 rounded-xl border border-slate-100">
                            ...
                            <p class="text-base font-black text-slate-900">{{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($item->qty * $item->harga, 0, ',', '.') }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <!-- Calculation Summary -->
        <div class="p-6 md:p-16 bg-slate-50/20">
            <div class="flex flex-col md:flex-row justify-between gap-12">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between p-4 bg-white border border-slate-200/60 rounded-xl shadow-sm">
                                <div class="flex flex-col">
                                    <span class="text-[11px] font-bold text-slate-900">{{ $payment->payment_date ? $payment->payment_date->format(\App\Models\Setting::get('date_format', 'd M Y')) : '-' }}</span>
                                    <span class="text-[9px] text-slate-400 uppercase mt-0.5">{{ $payment->payment_method }}</span>
                                </div>
                                <div class="flex items-center gap-4">
                                    <span class="text-xs font-black text-emerald-600">{{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($payment->amount, 0, ',', '.') }}</span>
                ...
                <div class="w-full md:w-80 space-y-4">
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span class="font-bold text-slate-900">{{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($invoice->subtotal, 0, ',', '.') }}</span>
                    </div>
                    @if($invoice->discount > 0)
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-500 font-medium">{{ app()->getLocale() == 'en' ? 'Discount' : 'Diskon' }} ({{ $invoice->discount_percent }}%)</span>
                        <span class="font-bold text-rose-500">- {{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($invoice->discount, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between items-center text-sm border-t border-slate-100/60 pt-2 pb-1">
                        ...
                        <span class="font-bold text-slate-900">{{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($invoice->subtotal - $invoice->discount, 0, ',', '.') }}</span>
                    </div>
                    @endif
                    @if($invoice->ppn > 0)
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-500 font-medium">PPN ({{ $invoice->tax_percent }}%)</span>
                        <span class="font-bold text-slate-900">+ {{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($invoice->ppn, 0, ',', '.') }}</span>
                    </div>
                    @endif
                    @if($invoice->pph > 0)
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-500 font-medium">PPh ({{ $invoice->pph_percent }}%)</span>
                        <span class="font-bold text-slate-900">+ {{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($invoice->pph, 0, ',', '.') }}</span>
                    </div>
                    @endif
                    ...
                    <div class="flex justify-between items-center p-4 bg-gold-50 rounded-xl border border-gold-100">
                        ...
                        <span class="text-sm font-black text-gold-700">{{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($invoice->amount_due, 0, ',', '.') }}</span>
                    </div>
                    @endif
                    ...
                    <div class="pt-6 border-t border-slate-200 flex justify-between items-center">
                        ...
                        <span class="text-xl md:text-3xl font-black text-gold-600 font-outfit">{{ \App\Models\Setting::get('currency_symbol', 'Rp') }} {{ number_format($invoice->total, 0, ',', '.') }}</span>
                    </div>
        ...
        <!-- Field Job Documentation Gallery -->
        @if($invoice->attachments->count() > 0)
        <div class="px-6 md:px-16 py-10 border-t border-slate-100 bg-slate-50/50">
            <h3 class="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                ...
                {{ app()->getLocale() == 'en' ? 'Field Job Documentation' : 'Dokumentasi Pekerjaan Lapangan' }}
            </h3>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                @foreach($invoice->attachments as $attachment)
                    <div class="group relative aspect-square rounded-2xl overflow-hidden border border-slate-200/60 bg-white shadow-sm hover:shadow-md transition-all duration-300">
                        <img src="{{ asset('storage/' . $attachment->file_path) }}" alt="Documentation" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                        <a href="{{ asset('storage/' . $attachment->file_path) }}" target="_blank" class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                            <span class="px-4 py-2 bg-white/95 text-slate-900 text-xs font-bold rounded-xl shadow-lg flex items-center gap-1.5 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                                <i data-lucide="maximize-2" class="w-3.5 h-3.5"></i>
                                {{ app()->getLocale() == 'en' ? 'View Full' : 'Lihat Penuh' }}
                            </span>
                        </a>
                    </div>
                @endforeach
        ...
        <div class="p-6 md:p-10" x-data="{
            ...
            <form action="{{ route('payments.store') }}" method="POST" class="space-y-6">
                ...
                <div class="pt-6 flex flex-col-reverse sm:flex-row items-center justify-end gap-3">
                    ...
                    <button type="submit" :disabled="!isValid" :class="{'opacity-50 cursor-not-allowed': !isValid}" class="w-full sm:w-auto px-8 py-3 bg-gold-500 text-slate-950 rounded-xl text-sm font-black shadow-lg shadow-gold-500/20 active:scale-95 transition-all">{{ app()->getLocale() == 'en' ? 'Save Payment' : 'Simpan Pembayaran' }}</button>
                </div>
            </form>
        </div>
    </x-modal>
    ...
    <div class="md:hidden fixed bottom-0 left-0 right-0 z-50 pointer-events-none p-4 flex justify-center">
        <div class="pointer-events-auto w-full bg-[#0F2A44] border border-slate-800 p-4 rounded-2xl flex gap-3 shadow-2xl items-center justify-between">
            ...
            <div class="flex gap-2">
                <a href="{{ route('contract-invoices.edit', $invoice->id) }}" class="px-4 py-2.5 bg-amber-500 text-slate-950 rounded-xl text-xs font-bold transition-all active:scale-95 flex items-center gap-1">
                    <i data-lucide="edit-3" class="w-3.5 h-3.5"></i>
                    <span>{{ __('ui.edit') ?? 'Edit' }}</span>
                </a>
                <button type="button" @click="$dispatch('download-pdf', { url: '{{ route('contract-invoices.pdf', $invoice) }}', filename: 'Invoice-{{ $invoice->invoice_number }}.pdf' })" class="px-4 py-2.5 bg-white text-[#0F2A44] rounded-xl text-xs font-bold transition-all active:scale-95 flex items-center gap-1">
