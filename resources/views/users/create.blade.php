<x-app-layout :title="app()->getLocale() == 'en' ? 'Team & Staff Management' : 'Manajemen Tim & Staf Operasional'">
    <div class="mb-10">
        <div class="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">
            <a href="{{ route('users.index') }}" class="hover:text-gold-600 transition-colors">{{ app()->getLocale() == 'en' ? 'Team Management' : 'Manajemen Tim' }}</a>
        ...
        <form action="{{ route('users.store') }}" method="POST" autocomplete="off" class="glass-card p-10 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap') }}" required class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200/60 rounded-lg text-sm outline-none focus:ring-2 focus:ring-gold-500/10">
                    @error('nama_lengkap')
                ...
                <div class="space-y-2">
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autocomplete="new-password" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200/60 rounded-lg text-sm outline-none focus:ring-2 focus:ring-gold-500/10">
                    @error('email')
                        <p class="text-xs text-rose-500 font-bold mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-2">
                ...
                <select name="peran_akses" required class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200/60 rounded-lg text-sm outline-none focus:ring-2 focus:ring-gold-500/10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    <div class="flex justify-between items-center">
                        ...
                        <button type="button" onclick="generatePassword()" class="text-[10px] font-bold text-gold-600 hover:text-gold-800 transition-colors flex items-center gap-1 uppercase tracking-wider">
                            <i data-lucide="key" class="w-3 h-3"></i>
                            <span>{{ app()->getLocale() == 'en' ? 'Generate' : 'Hasilkan' }}</span>
                        </button>
                    </div>
                    <div class="relative">
                        <input type="password" id="password" name="password" required autocomplete="new-password" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200/60 rounded-lg text-sm outline-none focus:ring-2 focus:ring-gold-500/10 pr-10">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input type="password" id="password_confirmation" name="password_confirmation" required autocomplete="new-password" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200/60 rounded-lg text-sm outline-none focus:ring-2 focus:ring-gold-500/10 pr-10">
