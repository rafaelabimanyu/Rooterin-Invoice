            ...
            <div class="flex-1 space-y-8 relative">
                ...
                @empty
                    <div class="text-center py-12 px-6">
                        ...
                        <p class="text-xs font-bold text-slate-900 uppercase tracking-widest">{{ __('dashboard.no_activities') }}</p>
                        <p class="text-[11px] text-slate-400 mt-2">{{ __('dashboard.activities_desc') }}</p>
                    </div>
                @endforelse
            </div>

            <div class="mt-10 pt-10 border-t border-slate-50">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 text-center">{{ __('dashboard.system_information') }}</p>
                <div class="p-5 bg-slate-50/50 rounded-2xl border border-slate-100 space-y-4">
                    <div class="flex justify-between items-center text-[11px]">
                        <span class="text-slate-500 font-bold">{{ __('dashboard.node_identity') }}</span>
                        <span
                            class="font-black text-slate-900">STAFF-{{ str_pad(auth()->id(), 4, '0', STR_PAD_LEFT) }}</span>
                    </div>
                    <div class="flex justify-between items-center text-[11px]">
                        <span class="text-slate-500 font-bold">{{ __('dashboard.session_integrity') }}</span>
                        <span class="font-black text-emerald-500 flex items-center gap-1.5">
                            <i data-lucide="check-circle" class="w-3.5 h-3.5"></i> {{ __('ui.aktif') }}
                        </span>
                    </div>
                </div>
            </div>
