    ...
    <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            <div class="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">
                ...
                <span class="text-slate-900 dark:text-white">{{ $client->kode_client }}</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white font-outfit">{{ $client->nama_client }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">{{ $client->nama_perusahaan }} — {{ $client->kota }}, {{ $client->provinsi }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div class="space-y-8">
            <div class="glass-card p-8">
                <h3 class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-widest mb-6">Contact Information</h3>
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        <i data-lucide="mail" class="w-4 h-4 text-slate-400"></i>
                        <span class="text-sm text-slate-600 dark:text-slate-400">{{ $client->email ?? 'No email set' }}</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <i data-lucide="phone" class="w-4 h-4 text-slate-400"></i>
                        <span class="text-sm text-slate-600 dark:text-slate-400">{{ $client->no_hp ?? 'No phone set' }}</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <i data-lucide="map-pin" class="w-4 h-4 text-slate-400"></i>
                        <span class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{{ $client->alamat }}</span>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="glass-card overflow-hidden">
                <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30">
                    <h3 class="font-bold text-slate-900 dark:text-white font-outfit">Invoice History</h3>
                </div>
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="text-[10px] font-bold uppercase tracking-widest text-slate-400 bg-slate-50/50">
                                ...
                                <th class="px-8 py-4 border-b border-slate-100 dark:border-slate-800">Date</th>
                                <th class="px-8 py-4 border-b border-slate-100 dark:border-slate-800">Total</th>
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50">
                            ...
                            <tr class="table-row-premium cursor-pointer" onclick="window.location='{{ route('invoices.show', $invoice) }}'">
                                <td class="px-8 py-4"><span class="text-[13px] font-bold text-slate-900 dark:text-white">{{ $invoice->invoice_number }}</span></td>
                                <td class="px-8 py-4"><span class="text-[12px] text-slate-500">{{ $invoice->tanggal_invoice->format('M d, Y') }}</span></td>
                                <td class="px-8 py-4"><span class="text-[13px] font-black text-slate-900 dark:text-white">Rp {{ number_format($invoice->total, 0, ',', '.') }}</span></td>
            ...
            <div class="glass-card overflow-hidden">
                <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30">
                    <h3 class="font-bold text-slate-900 dark:text-white font-outfit">Quotation History</h3>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead>
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50">
                            ...
                            <tr class="table-row-premium cursor-pointer" onclick="window.location='{{ route('quotations.show', $quotation) }}'">
                                <td class="px-8 py-4"><span class="text-[13px] font-bold text-slate-900 dark:text-white">{{ $quotation->quotation_number }}</span></td>
                                <td class="px-8 py-4"><span class="text-[12px] text-slate-500">{{ $quotation->tanggal_quotation->format('M d, Y') }}</span></td>
                                <td class="px-8 py-4"><span class="text-[13px] font-black text-slate-900 dark:text-white">Rp {{ number_format($quotation->total, 0, ',', '.') }}</span></td>
