...
>
        ...
        <div class="flex-1 min-w-[250px] snap-start shrink-0 md:shrink md:flex-initial">
            ...
            <div class="flex items-center gap-1.5 overflow-x-auto scrollbar-none py-0.5">
                ...
                <button wire:click="$set('status', 'paid')" type="button" 
                    class="px-3.5 py-2.5 rounded-xl text-xs font-bold border transition-all duration-200 active:scale-95
                    {{ $status === 'paid' ? 'bg-emerald-50 text-white border-emerald-500 shadow-md shadow-emerald-500/15' : 'bg-white/70 text-slate-600 border-slate-200/60 hover:bg-slate-50' }}"
                >
                    Paid
                </button>
    ...
    <template x-teleport="body">
        ...
        >
            ...
            >
                ...
                <div wire:loading wire:target="openAddModal, openEditModal, saveReminder, deleteReminder" class="absolute inset-0 bg-white/80 backdrop-blur-md z-50 flex flex-col items-center justify-center rounded-[32px] transition-all duration-350">
                    <div class="flex flex-col items-center gap-4">
                        <div class="relative flex items-center justify-center">
                            <div class="w-12 h-12 border-4 border-indigo-500/20 rounded-full absolute"></div>
                            <div class="w-12 h-12 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
                        </div>
                        <p class="text-[10px] font-black text-indigo-650 uppercase tracking-[0.2em] animate-pulse">{{ app()->getLocale() == 'en' ? 'Processing Event...' : 'Memproses Data...' }}</p>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-50">
                    ...
                    <button type="button" wire:click="$set('showModal', false)" class="text-slate-400 hover:text-slate-650 transition-colors p-1.5 hover:bg-slate-50 rounded-xl">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
    ...
    <template x-teleport="body">
        ...
        >
            ...
            >
                <!-- Premium Loading Micro-Interaction -->
                <div wire:loading wire:target="viewInvoiceDetails" class="absolute inset-0 bg-white/80 backdrop-blur-md z-50 flex flex-col items-center justify-center rounded-[32px] transition-all duration-350">
                    <div class="flex flex-col items-center gap-4">
                        <div class="relative flex items-center justify-center">
                            <div class="w-12 h-12 border-4 border-indigo-500/20 rounded-full absolute"></div>
                            <div class="w-12 h-12 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
                        </div>
                        <p class="text-[10px] font-black text-indigo-650 uppercase tracking-[0.2em] animate-pulse">{{ app()->getLocale() == 'en' ? 'Loading Details...' : 'Memuat Detail...' }}</p>
                    </div>
                </div>
                ...
                @if($viewedInvoice)
                    ...
                    <div class="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-4">
                        <button type="button" wire:click="$set('showInvoiceModal', false)" class="px-6 py-3 text-sm font-bold text-slate-500 hover:text-slate-900 transition-colors">{{ app()->getLocale() == 'en' ? 'Close' : 'Tutup' }}</button>
