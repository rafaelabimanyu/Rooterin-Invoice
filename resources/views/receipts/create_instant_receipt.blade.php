<x-app-layout :title="app()->getLocale() == 'en' ? 'Create Instant Receipt' : 'Buat Kwitansi Instan'">
    <div class="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            <div class="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
                ...
                <span class="text-slate-900">{{ app()->getLocale() == 'en' ? 'Create Instant Receipt' : 'Buat Kwitansi Instan' }}</span>
            </div>
            <h1 class="text-3xl font-bold text-slate-900 font-outfit leading-tight">{{ app()->getLocale() == 'en' ? 'Create Instant Receipt' : 'Buat Kwitansi Instan' }}</h1>
            <p class="text-slate-500 mt-1">{{ app()->getLocale() == 'en' ? 'Instantly issue a receipt and register a paid invoice in one step.' : 'Buat kwitansi lunas instan dan catat invoice otomatis dalam satu langkah.' }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('receipts.index') }}" @click="localStorage.removeItem('draft_receipt_data')" class="px-5 py-2.5 bg-white border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-all">
                {{ __('Discard') }}
            </a>
        </div>
    </div>
                ...
                <div class="bg-white p-10 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" value="{{ str_replace('INV-', 'KWT-', $invoice_number) }}" readonly class="w-full px-4 py-2.5 bg-slate-100 border border-slate-200 rounded-lg text-sm text-slate-500 font-mono cursor-not-allowed">
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-4">
                ...
                <div class="fixed bottom-0 left-0 w-full bg-[#1e293b] text-white p-4 shadow-2xl z-50 flex items-center justify-between border-t border-slate-800 pointer-events-none md:pointer-events-auto md:relative md:block md:p-10 md:rounded-xl md:shadow-2xl md:space-y-8 md:sticky md:top-24 md:z-auto md:border-t-0 animate-fade-in-up">
                    ...
                    <div class="flex items-center justify-between w-full pointer-events-auto md:block md:pt-6 md:border-t md:border-slate-700 md:space-y-6">
                        <div>
                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none md:text-[10px] md:mb-2">{{ app()->getLocale() == 'en' ? 'Grand Total Paid' : 'Total Pembayaran' }}</p>
                        ...
                        <button type="submit" class="pointer-events-auto px-5 py-3 bg-gold-500 hover:bg-gold-600 text-slate-950 rounded-xl font-black text-[12px] uppercase tracking-widest flex items-center gap-2 active:scale-95 transition-all md:w-full md:py-4 md:rounded-lg md:text-[13px] md:justify-center md:shadow-lg md:shadow-gold-500/20">
                            ...
                            {{ app()->getLocale() == 'en' ? 'Issue & Download PDF' : 'Buat & Unduh PDF' }}
                        </button>
                    </div>
        ...
        function invoiceForm() {
            return {
                ...
                init() {
                    ...
                    this.$nextTick(() => {
                        const formEl = document.querySelector('form[action="{{ route('receipts.store_instant') }}"]');
                        if (formEl) {
