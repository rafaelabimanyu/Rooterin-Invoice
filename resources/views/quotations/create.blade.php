    ...
    <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            <div class="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">
                ...
                <span class="text-slate-900 dark:text-white">New Proposal</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white font-outfit">Create Quotation</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Generate a professional price offer for your client.</p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div class="lg:col-span-8 space-y-8">
                <div class="glass-card p-10">
                    <h3 class="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-widest mb-8 pb-4 border-b border-slate-50 dark:border-slate-800">1. Client & Dates</h3>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-2">
                            <label class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Client Account</label>
                            <select name="client_id" required class="w-full px-4 py-2.5 bg-slate-50/50 dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/10 transition-all">
                        ...
                        <div class="space-y-2">
                            <label class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Quotation Number</label>
                            <input type="text" name="quotation_number" value="{{ $quotation_number }}" readonly class="w-full px-4 py-2.5 bg-slate-100 dark:bg-slate-800/50 border border-slate-200/60 rounded-lg text-sm text-slate-500 font-mono cursor-not-allowed">
                        </div>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                        <div class="space-y-2">
                            <label class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Proposal Date</label>
                            <input type="date" name="tanggal_quotation" value="{{ date('Y-m-d') }}" required class="w-full px-4 py-2.5 bg-slate-50/50 dark:bg-slate-800 border border-slate-200/60 rounded-lg text-sm text-slate-900 dark:text-white outline-none">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Expiry Date</label>
                            <input type="date" name="expiry_date" value="{{ date('Y-m-d', strtotime('+14 days')) }}" required class="w-full px-4 py-2.5 bg-slate-50/50 dark:bg-slate-800 border border-slate-200/60 rounded-lg text-sm text-slate-900 dark:text-white outline-none">
                        </div>
                    </div>
                </div>

                <div class="glass-card overflow-hidden">
                    <div class="px-10 py-6 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between">
                        <h3 class="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-widest">2. Service Items</h3>
                    ...
                    <div class="p-10 space-y-6">
                        <template x-for="(item, index) in items" :key="index">
                            <div class="relative grid grid-cols-1 md:grid-cols-12 gap-6 pb-6 border-b border-slate-50 dark:border-slate-800 last:border-0 last:pb-0">
                                <div class="md:col-span-6 space-y-2">
                                    <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Description</label>
                                    <input type="text" :name="`items[${index}][deskripsi]`" x-model="item.deskripsi" required class="w-full bg-transparent border-none p-0 focus:ring-0 text-[13px] text-slate-900 dark:text-white font-semibold">
                                </div>
                                ...
                                <div class="md:col-span-3 space-y-2 text-right">
                                    ...
                                    <div class="text-[13px] font-black text-slate-900 dark:text-white" x-text="formatCurrency(item.qty * item.harga)"></div>
                                </div>
                                <button type="button" @click="removeItem(index)" x-show="items.length > 1" class="absolute -right-4 top-1/2 -translate-y-1/2 p-1 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded">
            ...
            <div class="lg:col-span-4 space-y-8">
                <div class="bg-[#1e293b] text-white p-10 rounded-xl shadow-2xl space-y-8 sticky top-24">
                    <h3 class="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Proposal Summary</h3>
